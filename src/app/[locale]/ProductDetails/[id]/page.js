...
import {
  ...
  menuItems,
  options,
} from "@/app/[locale]/categories/data";
...
const Page = ({ params }) => {
  ...
  useEffect(() => {
    ...
    const selectedBranch = shop?.brunchs?.find(
      (branch) => branch.id === Number(branchId)
    );
    const selectedCat = selectedBranch?.cat_meal?.find(
      (cat) => cat.id === Number(catId)
    );
    const selectedMeal = selectedCat?.meals?.find(
      (meal) => meal.id === Number(id)
    );
  ...
  const handlePrice = () => {
    ...
    const { itemDiscount, itemSubTotal, itemTax } =
      itemPriceDetailsCalculation(meal);
  ...
  const handleSelectVariants = (currentOptions) => {
    if (selectedOptions?.some((option) => option.id === currentOptions.id)) {
      const newOptions = selectedOptions.filter(
        (option) => option.id !== currentOptions.id
      );
  ...
  const handleSelectExtra = (currentExtra) => {
    if (selectedExtra?.some((extra) => extra.id === currentExtra.id)) {
      const newExtra = selectedExtra.filter(
      ...
      {isLoading ? (
        ...
        <>
          ...
          >
            ...
            <Box sx={{ zIndex: "5000", padding: "0px 20px" }}>
              ...
              <Box width={"100%"} height={"60px"}>
                ...
                >
                  <Box sx={{ display: "flex", alignItems: "center" }}>
                    <span
                      className="icon-star"
                      style={{ color: "#ef7d00", fontSize: "14px" }}
                    ></span>
                    <Typography
                      variant="body2"
                      color="white"
                  ...
                  <Box display="flex" alignItems="center">
                    <span className="icon-calories-1">
                  ...
                  <Box display="flex" alignItems="center">
                    <span className="icon-chronometer">
                      <span className="path1"></span>
                      <span className="path2"></span>
                      <span className="path3"></span>
                      <span className="path4"></span>
          ...
          {/* الجزء المتحرك  */}
          <Box
            sx={{ position: "relative", top: "340px", padding: "12px 20px" }}
          >
            <Box display="flex" alignItems="center">
              {!specialID && (
                <Typography
                    ...
                    <>
                      ...
                      <Button
                        ...
                        sx={{
                          ...
                          margin: "5px",
                          backgroundColor:
                            selectedSize === size ? "#797993" : "#302E3B",
            ...
            <Box sx={{ marginTop: "15px" }}>
              ...
              >
                {t("options")}{" "}
                <span
                  style={{ fontSize: "9px", fontWeight: "300", color: "white" }}
                >
                  {t("required)")}
                </span>
              </Typography>
              ...
              <Box display="flex" flexWrap="wrap" gap={1}>
                {mealData?.variants?.map((option, index) => (
                  ...
                  >
                    ...
                    >
                      {option.price === 0 ? (
                        <span style={{ fontWeight: 400 }}>{t("free")}</span>
                      ) : (
                        `+${option.price} EGP`
                      )}
                    </Typography>
                ...
                {mealData?.extras?.map((extra, index) => (
                  <Button
                    ...
                    sx={{
                      ...
                      width: "47%",
                      height: "25px",
                      backgroundColor: selectedExtra?.some(
                        (item) => item.id === extra.id
                      )
                        ? "#48485B"
                        : "#1E1E2A",
                      color: selectedExtra?.some((item) => item.id === extra.id)
                        ? "white"
                        : "#AAAAAA",
                      border: selectedExtra?.some(
                        (item) => item.id === extra.id
                      )
                        ? "none"
                        : "1px solid #48485B",
            ...
            <Box sx={{ marginTop: "15px", marginBottom: "120px" }}>
              ...
              <Typography
                variant="body2"
                sx={{ fontSize: "11px", color: "#AAAAAA", width: "100%" }}
              >
          ...
          >
            <Box>
              <Typography
                variant="h6"
                sx={{ fontSize: "11px", fontWeight: "bold", color: "gray" }}
              >
                {t("price")}
              </Typography>
              <Typography
              ...
              >
                {selectedSize && count !== 0 && !specialID && (
            ...
            <Box sx={{ width: "46%" }}>
              <Button
                ...
                sx={{
                  backgroundImage:
                    "linear-gradient(to right, #302E3B, #797993)",
                  color: "white",
                  textTransform: "capitalize",
                  fontSize: "12px",
                  float: "right",
              ...
              >
                {t("addToCart")}{" "}
                <span style={{ fontSize: "17px", marginLeft: "10px" }}>+</span>
              </Button>
            </Box>
          </Box>
        </>
