...
const page = () => {
  ...
  const prefetchTarget = (id) => {
    ...
    // build new URL
    const catIdUrl = `/categories/${id}?${params.toString()}`;
      ...
      >
        <Box display="flex" justifyContent="space-between" alignItems="center">
          {shopImg ? (
            ...
            <Typography
              variant="h6"
              sx={{ fontSize: "18px", fontWeight: "900", color: "#797993" }}
            >
              {t("logo")}
            </Typography>
          )}
        ...
        >
          <span
            ...
            style={{ marginRight: "6px", color: "#797993" }}
          ></span>
          <TextField
            variant="standard"
          ...
          />
          <span
            className="icon-settings-sliders"
            style={{ fontSize: "20px", color: "#797993" }}
          ></span>
        </Box>
      </Box>
...
export const handleSpecialOfferClick = (
  router,
  branchId,
  shopId,
  mealId,
  ...
  const categoryWithMeal = currentBranch.cat_meal.find((category) => {
    return category.meals.some((meal) => meal.id === Number(mealId));
  });
  if (shopId && branchId && categoryWithMeal) {
    router.push(
      `/ProductDetails/${mealId}?shopId=${shopId}&branchId=${branchId}&catId=${categoryWithMeal.id}&special=${specialOfferId}` +
...
const CategoryView = ({
  ...
  const handlCatClick = (id) => {
    if (shopId && branchId) {
      let catIdUrl =
        `/categories/${id}?shopId=${shopId}&branchId=${branchId}` +
        (tableId ? `&tableId=${tableId}` : "");
  ...
  return (
    <Box mt={3}>
      ...
      <div
        ...
        style={{ marginTop: "5px", marginBottom: "70px" }}
      >
...
const GridView = ({
  ...
  useEffect(() => {
    if (
      Array.isArray(currentBranch?.cat_meal) &&
      currentBranch?.cat_meal.length > 0
    ) {
  ...
  return (
    ...
    >
      {currentBranch?.cat_meal?.map((item, index) => {
        ...
        const textColor =
          isActive && item.name !== "Popular" ? "white" : "#797993";
        const textColor2 =
          isActive && item.name !== "Popular" ? "white" : item?.color;
            ...
            >
              ...
              >
                <img
                  ...
                  style={{ maxWidth: "50px", aspectRatio: "1/1" }}
                />
              </ListItemIcon>
