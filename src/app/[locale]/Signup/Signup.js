...
import { useGetCountry } from "@/hooks/governorates/useGetCountry.js";
...
export const Signup = () => {
  ...
  const handleApiError = (errorResponse) => {
    // handleApiError {status: 'error', message: 'بعض البيانات غير مكتملة أو غير صحيحة.', errors: {…}}
    console.log("handleApiError", errorResponse);
    // Check if errorResponse and errorResponse.data exist
    ...
    // Navigate through errors object and show each error
    if (errors && typeof errors === "object") {
      Object.keys(errors).forEach((field) => {
        const fieldErrors = errors[field];

        if (Array.isArray(fieldErrors)) {
          // If errors is an array, show each error
      ...
      <Box
        sx={{
          ...
          "::before": {
            ...
            height: "100%",
            background:
              "linear-gradient(to top right,rgba(230, 127, 0, 0.541),rgba(54, 43, 57, 0.582),rgba(54, 43, 57, 0.582), rgba(33, 33, 63, 0.5))",
            zIndex: 6,
          },
        }}
      >
        <Box
          sx={{
            ...
            alignItems: "center",
            textAlign: "center",
            padding: { xs: "10px 20px", sm: "15px 40px", md: "20px 60px" },
          }}
        >
          <IconButton>
            <Link href="/marketing">
        ...
        >
          ...
          >
            ...
            {apiSuccess && (
              <Typography sx={{ color: "green", fontSize: "13px" }}>
                {apiSuccess}
              </Typography>
            )}
        ...
        >
          @ 2024 All Rights Reserved by Q-Tap
        </Typography>
      </Box>
    </Box>
  );
