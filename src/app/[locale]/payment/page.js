...
const page = () => {
  ...
  const makeOrder = async () => {
    try {
      ...
      if (formdata.servingWay === "dine_in") {
        data = {
          ...data,
          table_id: tableId || selectedTable,
          type: "dinein",
        };
      }
      console.log(JSON.stringify(data));
      const response = await axios.post(`${BASE_URL}add_orders`, data, {
        headers: {
      ...
      if (payWay === "wallet" && response.data.payment_url)
        localStorage.setItem(
          "payment_url",
          JSON.stringify(response.data.payment_url)
        );
      if (response) {
  ...
  useEffect(() => {
    if (formData && formData.servingWay === "delivery") {
      let pos = [formData?.userPosition?.[0], formData?.userPosition?.[1]];
      ...
      case "dine_in":
        return (
          <span
            ...
            style={{ fontSize: "20px", color: "#F78822" }}
          ></span>
        );
    ...
    >
      ...
      >
        <Box display="flex" flexDirection="column" sx={{ padding: "30px" }}>
          <Typography
            variant="body1"
            sx={{ fontSize: "12px", display: "flex", letterSpacing: 1 }}
          >
            {t("items")}
          </Typography>
          {cartItems &&
              ...
              <Box>
                <Box>
                  ...
                  <Typography
                    ...
                    display={"flex"}
                  >
                    {formData.servingWay} ,{renderIcon()}
                  </Typography>
                </Box>
                {formData?.servingWay !== "dine_in" && (
                  <Typography color="#AAAAAA" fontSize="12px">
                    <span style={{ color: "#797993", marginLeft: "10px" }}>
                      {t("address")} :{" "}
                    </span>
                    {formData.address}
                  </Typography>
            ...
            <span style={{ color: "#AAAAAA", fontSize: "11px", borderBottom: "1px solid #AAAAAA" }}>
              ...
              >
                <Box sx={{ width: "100%" }}>
                  ...
                  >
                    {t("tax")}:{" "}
                    <span style={{ color: "#AAAAAA" }}>{tax} EGP</span>
                  </Typography>
                  ...
                  >
                    {t("disCount")}:{" "}
                    <span style={{ color: "#AAAAAA" }}>{discount} EGP</span>
                  </Typography>
                </Box>

                <Box
                  sx={{ width: "100%", textAlign: "left", marginLeft: "70px" }}
                >
                  <Typography
                    variant="h6"
                    sx={{
      ...
      >
        ...
        >
          ...
          >
            {t("confirm")}{" "}
            <img src="/assets/hands.svg" style={{ marginLeft: "5px" }} />
          </Button>
          {/* </Link> */}
          <Button
          ...
          >
            <span
              className="icon-printer1"
              style={{ fontSize: "22px", color: "#AAAAAA" }}
            ></span>
          </Button>
        </Box>
      </Box>
